...
export async function addToCart(req, res) {
  try {
    ...
    if (game.owner_id === userId) {
      return res
        .status(400)
        .json({ message: "Du kan inte lägga din egen annons i korgen" });
    }
    ...
    if (type === "rent") {
      ...
      if (game.price_rent_per_month == null) {
        return res
          .status(400)
          .json({ message: "Spelet är inte möjligt att hyra" });
      }
    } else if (type === "buy") {
  ...
  try {
    ...
    for (const it of items) {
      ...
      if (g.status !== "available") {
        errors.push({
          cart_id: it.id,
          reason: "Annons är inte tillgänglig längre",
        });
        continue;
      }
      ...
      if (it.type === "buy") {
        ...
        await client.query("UPDATE games SET status = $1 WHERE id = $2", [
          "sold",
          g.id,
        ]);
