...
<template>
  <div class="modal-overlay">
    <div class="modal-card" style="max-width: 400px">
      <!-- HEADER -->
      <header class="row" style="gap: 0.25rem">
        <div
          class="card-title"
          style="
            display: flex;
        ...
        >
          ...
          <button
            class="btn"
            style="font-size: 0.7rem; line-height: 1.2; padding: 0.3rem 0.5rem"
            @click="$emit('close')"
          >
            Stäng
          </button>
        ...
        <div class="card-sub">{{ game.title }} • {{ game.platform }}</div>
      </header>
      ...
      <div class="row" style="gap: 1rem">
        ...
        <div class="card-sub" style="font-size: 0.8rem; white-space: pre-line">
          {{ game.description }}
        </div>
        ...
        >
          <div class="row" style="gap: 0.5rem">
            <div class="card-sub">
              <strong>Pris:</strong>
              <span v-if="game.price_sell != null"
                >{{ game.price_sell }} kr</span
              >
              <span v-else>-</span>
            </div>
        ...
        <div v-else class="row" style="gap: 1rem">
          <div class="input-group">
            ...
            />
            <p class="card-sub" style="font-size: 0.7rem; margin: 0">
              Minsta hyrestid är 1 månad.
            </p>
          </div>

          <div class="card-sub" style="font-size: 0.8rem">
            ...
            <p class="card-sub" style="font-size: 0.75rem; margin: 0.25rem 0 0">
              Status ändras till <em>{{ statusLabel("rented") }}</em> först vid
              betalning i kassan.
            </p>
          </div>
        </div>
        ...
        <!-- FEL / INFO -->
        <p v-if="errMsg" style="color: #fca5a5; font-size: 0.75rem; margin: 0">
          {{ errMsg }}
        </p>
      </div>
