...
<template>
  <section class="row" style="gap: 2rem">
    ...
    <div v-else class="card table-wrapper">
        ...
        <tbody>
          <tr v-for="it in cart.items" :key="it.key">
            ...
            <td>
              <span
                class="badge"
                :class="it.type === 'buy' ? 'success' : 'warn'"
              >
            ...
            <td>
              ...
              <template v-else>
                {{ it.rental_months }} × {{ it.unit_price }} kr =
                <strong>{{ it.total_price }} kr</strong>
              </template>
            </td>
            <td>
              <button class="btn danger" @click="remove(it.key)">
                Ta bort
              </button>
            </td>
          </tr>
      ...
      <p
        v-if="errMsg"
        style="color: #fca5a5; font-size: 0.8rem; margin: 0.75rem 0 0"
      >
        {{ errMsg }}
      </p>
      <p v-if="noteMsg" class="card-sub" style="margin: 0.25rem 0 0">
        {{ noteMsg }}
      </p>
    </div>
  </section>
